...
def generateTopoFile(topoFilename, topoDict):
    ...
    for pathInfo in topoDict.get(PATHS, DEFAULT_PATHS):
        ...
        if QUEUING_DELAY in pathInfo:
            ...
            queueSize = str(pathInfo.get(QUEUE_SIZE, int(max(DEFAULT_QUEUING_DELAY * bandwidthDelayProductDividedByMSS(bandwidth, delay, DEFAULT_MTU),
                                                             10))))
        loss = str(pathInfo.get(LOSS, DEFAULT_LOSS))
        print("path_c2r_" + str(pathNumber) + ":" + delay + "," + queueSize + "," + bandwidth + "," + loss, file=topoFile)
        # Added a hardcoded c2r_ as a linktype to pass the usage of the latest Minitopo for the current experiment
    ...
    if len(topoDict.get(NETEM, DEFAULT_NETEM)) > 0:
        for netemInfo in topoDict.get(NETEM, DEFAULT_NETEM):
            ...
            print("_netemAt_" + str(netemInfo[0]) + ":" + str(netemInfo[1]) + "," + str(netemInfo[2]), file=topoFile)
            # Name changed from 'netemAt_' to '_netemAt_' to adapt to the latest Minitopo usage
    topoFile.close()
